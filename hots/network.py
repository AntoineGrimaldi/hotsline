                    ...
                    if ts_batch_size:
                        for L in range(len(self.tau)):
                            ...
                            while events.shape[0]-start_indice.item()>ts_batch_size:
                                ...
                                all_ts, ind_filtered, previous_timestamp = timesurface(events, (self.sensor_size[0], self.sensor_size[1], self.n_pola[L]), ordering, tau = self.tau[L], surface_dimensions=[2*self.R[L]+1,2*self.R[L]+1], filtering_threshold = filtering_threshold[L], ts_batch_size = ts_batch_size, first_indice = start_indice, previous_timestamp = previous_timestamp, device = device)
