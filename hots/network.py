...
class network(object):
    ...
    def __init__(self,  name,
        ...
        # pooling
        if pooling_coef:
            self.channel_size = [(sensor_size[0]//np.prod(self.pooling_coef[:L]), sensor_size[1]//np.prod(self.pooling_coef[:L])) for L in range(1,nb_layers)]
            self.channel_size.insert(0,(sensor_size[0], sensor_size[1]))
            for L in range(nb_layers):
                assert (np.prod(self.pooling_coef[:L]))**2 <= self.channel_size[L][0]*self.channel_size[L][1]
        else:
            self.channel_size = [(sensor_size[0], sensor_size[1]) for L in range(nb_layers)]
            ...
            with torch.no_grad():
                ...
                for events, target in tqdm(loader):
                    ...
                    for L in range(len(self.tau)):
                        ...
                        if self.pooling_coef:
                            events[:,x_index] = torch.div(events[:,x_index], (self.pooling_coef[L]), rounding_mode='floor')
                            events[:,y_index] = torch.div(events[:,y_index], (self.pooling_coef[L]), rounding_mode='floor')
